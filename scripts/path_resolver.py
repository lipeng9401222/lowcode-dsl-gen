...
def cli():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="(默认开启) 检测到老结构 <apptag>/metadata/ 时直接非零退出，避免新建工作流落到老路径",
    )
    parser.add_argument(
        "--allow-legacy-metadata",
        action="store_true",
        help="兼容老应用：允许返回 <apptag>/metadata/ 老结构路径（不推荐，仅做迁移过渡）",
    )
    args = parser.parse_args()
    if args.allow_legacy_metadata:
        args.reject_legacy = False
    ...
    if args.mode in ("auto", "find"):
        ...
        if existing:
            existing_str = str(existing).replace("\\", "/")
            is_legacy = (
                "/metadata/" in existing_str + "/"
                or existing.name == "metadata"
                or (existing / "metadata").is_dir()
            )
            if is_legacy and args.reject_legacy:
                print_err(
                    f"❌ 检测到老结构应用根：{existing}\n"
                    f"   spec v2 已废弃 metadata/ 层。请先迁移：\n"
                    f"     mv {existing}/metadata/* {existing}/ && rmdir {existing}/metadata\n"
                    f"   迁移后重新运行；或临时使用 --allow-legacy-metadata 兼容（不推荐）。"
                )
                return 1
    ...
    # spec v2 路径红线：计算结果不允许包含 /metadata/ 段
