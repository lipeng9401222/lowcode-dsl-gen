...
def build_field(
    *,
    ...
    if field_type not in VALID_MIS_TYPES:
        raise ValueError(f"type '{field_type}' 不在白名单内: {sorted(VALID_MIS_TYPES)}")
    ...
    return normalize_mis_field(field, order_index=order_index)
...
def build_primary_key_field() -> dict:
    ...
    f = build_field(
        ...
        overrides=primary_key_overrides(),
        order_index=0,
    )
    return f
        ...
        if args.fields_json:
            ...
            for f in extras:
                ...
                if field_name in existing_names:
                    print_warn(f"字段 '{field_name}' 已自动生成（如主键 rowguid），跳过重复")
                    continue
                initial_fields.append(build_field(
                    name=field_name,
                    field_type=f.get("type", "nvarchar"),
                    description=f.get("description", ""),
                    length=int(f.get("length", default_length_for_type(f.get("type", "nvarchar")))),
                    overrides={k: v for k, v in f.items()
                               if k not in ("name", "type", "description", "length")},
                    order_index=len(initial_fields),
                ))
    ...
    if args.normalize_existing:
        ...
        for index, field in enumerate(fields):
            if not isinstance(field, dict):
                ...
                continue
            normalized = normalize_mis_field(field, order_index=index)
            if normalized.get("name") == "rowguid":
                normalized.update(primary_key_overrides())
    ...
    if args.fields_json:
        ...
        for f in extras:
            new_fields.append(build_field(
                ...
                overrides={k: v for k, v in f.items()
                           if k not in ("name", "type", "description", "length")},
                order_index=len(existing_fields) + len(new_fields),
            ))
    elif args.name:
        overrides = {
        ...
        new_fields.append(build_field(
            ...
            length=args.length,
            overrides=overrides,
            order_index=len(existing_fields),
        ))
    else:
