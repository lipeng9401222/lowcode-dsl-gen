...
│  · 计划文档与对话追溯门禁(.lowcode-plans/<apptag>-plan.md)             │
└──────────────────────────────┬──────────────────────────────────────────┘
                               ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  ③ 知识层(references/)                                                 │
...
```mermaid
flowchart TD
    A[用户进入 skill] --> PLAN0[创建/更新 .lowcode-plans 计划文档]
    PLAN0 --> B{识别触发模式}
    B -->|D 整应用生成| D1[读 whole-app-workflow.md]
    B -->|A 从零创建| A1[读 00 + 01]
    B -->|B 修改/补充| B1[读 00 + 对应 0X]
    B -->|C 资产快速通道| C1[读 对应 0X]
    B -->|信息不足| Q1[追问并记录问题选项 不要猜]

    D1 --> S0[8 阶段渐进确认 每轮写对话确认记录]
    A1 --> S0
    B1 --> S0
    C1 --> SC[资产级最小问询]
    ...
    S6 --> S7[workflow 可选]
    S7 --> S8[event 可选]
    S8 --> P[整理生成计划章节<br/>approval_status=pending]

    SC --> P

    P --> APP{用户明确批准?}
    APP -->|否/模糊| Q2[继续追问并回填确认记录 不要落盘]
    Q2 --> P
    APP -->|是| EX[执行装配脚本]
...
```
 第一轮   ──→  创建/更新 .lowcode-plans/<apptag>-plan.md
                          │
 每次提问前 ─→  追加对话确认记录(status=awaiting_user)
                          │   阶段 / 问题 / 选项 / 推荐项 / 提问方式
                          │
 用户回复后 ─→  回填同一条记录
                          │   用户原文 / 解析结果 / 确认状态 / 阶段影响
 ...
 批准前   ──→  只能写计划文档(其它一律拒写)
                          │
                          │ 计划文档必含:
                          │   tool_name / plan_revision / created_at / updated_at
                          │   last_interaction_id / current_stage / confirmed_stages
...
Plan Mode / 结构化问答也必须写入计划文档,不能只依赖宿主会话历史。

非交互模式(`codex exec` / CI):停在已写入计划文档且等待人工确认的步骤,回报"等待人工批准"。
...
| pagedesigne | `page/` | json | `add_page.py` | `page/页面设计器/index.md` + 子目录 | 无 |
...
|---------|------|------------|
| `method` | 流程外部 Java 方法或动作流标识 | 可被 `workflowEvent.eventmethodguid` / `workflowTransitionCondition.methodguid` 引用；动作流场景使用 `ruleguid` |
...
├────────────────────────────────────────────────────────────┤
│  · 单文件:python3 validate_yml.py <file>                    │
│  · 整应用:python3 validate_yml.py --strict --check-refs <app-root> │
│                                                              │
│  校验项分类:                                                  │
...
7. **workflow 数据表只走 sql_tablename** —— `workflowPvMisTableSet` 不生成旧 ID 字段，必须用 `sql_tablename` 匹配同应用 `mis.tableName`
...
每次发布前:
- 跑 `python3 scripts/validate_yml.py --strict --check-refs <一组测试应用根目录>`
- 跑 `evals/` 下样本回归
...
- **数据表映射入口收敛**：`workflowPvMisTableSet` 只通过 `sql_tablename` 关联 MIS 表，生成入口不再暴露旧 ID 参数
- **已废弃数据表 ID 字段清理**：旧数据表 ID 写法不再作为 workflow 生成能力暴露，避免模型把业务表名误写成 ID 字段
...
### 验证结果
- `validate_yml.py` 成功检出 `用车申请审批流程.workflow.yml` 的旧数据表 ID 字段错误
- `add_workflow.py --help` 确认工作流只保留 `--sql-tablename` 作为 MIS 关联入口
- Python 语法检查通过
