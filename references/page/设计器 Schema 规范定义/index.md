...
2. 根据页面类型读取示例：列表页读 `列表示例.md`，表单/详情页读 `表单示例.md`。
3. 按需读取结构规范：页面根对象读 `02-页面 Schema.md`，组件层级读 `03-视图树.md`，字段和模型读 `04-视图模型.md`。
4. 需要接口、动作流或页面事件时，再读 `05-资源.md` 和 `06-动作与事件.md`。
5. 落盘前必须读 `08-规范化与校验.md`，并运行校验脚本。
