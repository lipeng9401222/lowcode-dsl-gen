...
metadata_path = "/".join(parts)
```

> **该算法已实现**于 `scripts/path_resolver.py`，**不要在对话里手算**。
...
| metadata 子目录 | **固定的 7 个名字**，不可变 | `codeitem/`、`mis/`、`module/`、`event/`、`api/`、`workflow/`、`pagedesigne/` |
...
调用 `python scripts/validate_yml.py <metadata 目录>` 执行上述校验。
...
定位后调用 `scripts/path_resolver.py --apptag <tag>` 算出 metadata 目录绝对路径，并列出现有资产清单。
...
5. 用户明确批准后才落盘；落盘后执行 `python scripts/validate_yml.py --check-refs <metadata目录>`。
...
# 2. 给定 apptag 算 metadata 绝对路径
python scripts/path_resolver.py \
  --apptag xmlx \
  --workspace /path/to/workspace

# 3. 在指定 metadata 目录追加一个代码项
...
# 9. 列出现有 metadata 资产清单
python scripts/inventory_metadata.py \
  --metadata /path/to/metadata

# 10. 静态校验某个文件或整个 metadata 目录（含跨引用校验）
python scripts/validate_yml.py --check-refs /path/to/metadata
```
