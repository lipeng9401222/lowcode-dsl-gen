...
5. 用户明确批准后才落盘；落盘后执行 `python scripts/validate_yml.py --check-refs <app-root>`，**交付前最终校验**统一改用 `python scripts/validate_yml.py --strict --check-refs <app-root>`（spec v2 严格模式：旧驼峰键 / `tableid` 残留 / `metadata/` 老结构 一律升级为 error）。
...
python scripts/validate_yml.py --strict --check-refs /path/to/<apptag>
```
