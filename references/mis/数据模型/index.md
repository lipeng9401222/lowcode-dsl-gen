...
- 不在本资产范围内的事：
  ...
  - 业务流程审批属于 `workflow`（通过 `workflowPvMisTableSet.sql_tablename` 引用同应用 `mis.tableName`），详见 `references/workflow/工作流/index.md`
...
- 被其他资产引用：
  - `workflow` 的 `workflowPvMisTableSet.sql_tablename` 引用同应用 `mis.tableName`
  - `event`/`pagedesigne` 通过 `tableName` 或数据源配置引用 mis（数据源/表单）
...
> 生成后 YAML 必须落成完整字段结构。对话和 `--fields-json` 入参可以只给 `name/type/description/length` 等关键项，但脚本必须补齐模板默认属性，不能交付只含少量键的“瘦字段”。

#### 生成后必备模板属性

以下字段对应 `mis.md` 示例里的中文备注字段，最终 `fields[]` 每个字段都必须包含：

`name/type/description/length/datasourceCodename/fielddisplaytype/isframeou/isframeuser/isforeignkey/autoincrease/mustfill/defaultvalue/uniquefield/notnull`
...
每次追问表名、字段、约束或关联关系前，都要先把问题和选项写入计划文档的 `对话确认记录`；用户回复后回填确认结果，再决定是否进入批准。
...
直接调用 `python3 scripts/add_mis_field.py --mis <yml绝对路径> --name customer_phone --type nvarchar --length 50 --description "客户固定电话"`。
...
> **关键经验**：对话和 JSON 入参不要拼命填全部 30+ 属性，**只显式传"非默认"的部分**；但最终落盘的 yml 必须是完整字段结构。`scripts/add_mis_field.py` 会自动用模板默认值补齐。
...
| 每个字段包含必备模板属性（如 `datasourceCodename/fielddisplaytype/isframeou/.../notnull`） | 默认 warn，`--strict` 下非 0 |
| 字段 `type` 在合法枚举内 | 枚举校验 |
...
```bash
# 创建 mis 表（最终落盘会自动补齐完整字段结构）
python3 scripts/add_mis_field.py \
  --app-root <app-root> \
  --table customerinfo \
  --table-desc "客户信息表" \
  --create \
  --fields-json '[{"name":"customer_name","type":"nvarchar","description":"客户姓名","length":50}]'
