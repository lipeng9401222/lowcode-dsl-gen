...
1. **批准前唯一可写文件**：`<工程根>/.lowcode-plans/<apptag>-plan.md`；apptag 暂时未定时用 `<任务简称>-plan.md`，确认 apptag 后重命名。**计划文档严禁落入 skill 仓库本身**（skill 仓库根的 `*-plan.md` 已在 `.gitignore` 中拦截）。
2. **计划文档必须包含字段**：
...
3. **目标路径必须用脚本计算**（spec v2 路径红线，重要！）：
   - 应用根目录必须是 `python scripts/path_resolver.py --apptag <tag>` 的输出。
   - **任何形如 `<apptag>/metadata/<asset>/...` 的路径都是老结构，spec v2 已废弃。**
   - 计划文档里如果写了 `metadata 目录` 这种字眼，必须改写成 `应用根目录`。
   - 脚本会校验：当 `--app-root` 路径包含 `/metadata/` 段时直接报错退出，不能落盘。
4. **`approval_status` 仅在用户明确说"批准创建 / 按计划生成 / 可以落盘"等语义后**改为 `approved`，并把用户原文记录到 `approval_text`。模糊回复（"嗯"、"好像可以"、"你看着办"）**不算批准**，继续追问。
5. **`pending` 状态下禁止**调用 `scripts/scaffold_app.py`、`scripts/add_codeitem.py`、`scripts/add_mis_field.py`、`scripts/add_event.py`、`scripts/add_module.py`、`scripts/add_workflow.py`、`scripts/add_pagedesigne.py`，**禁止写应用资产**。
6. **非交互模式**（如 `codex exec` / CI / 无人值守 session）：若无法等待用户批准，停在生成计划文档这一步，回报"等待人工批准"，**不要假装已批准**。
7. **资产级快速通道也走计划文档门禁**："快速"只表示减少追问范围，不表示跳过计划和批准。
...
| `workflow` | 用户**明确要求**审批流/工作流；需确认流程能力、外部方法/事件是否关联动作流 ruleGuid、自由流程是否禁用退回；**[spec v2 触发词]** 当用户提到下列关键词时，必须主动追问后用对应入参调 `add_workflow.py`，不能默认跳过：<br>- "字段权限 / 只读字段 / 字段隐藏 / 必填字段" → `--activity-materials-json`<br>- "通过率 / 多人会签 X% / 60% 通过 / 未通过退回到开始" → `--approve-pass-rate-json`<br>- "超时提醒 / 短信通知 / 多人锁定 N 分钟 / 移动端小程序规则 / 多路汇合 / 自定义汇合" → `--activity-extra-json`<br>- "金额>X 走A否则走B / 条件分支 / 流转条件" → `--conditions-json` + `--contexts-json`<br>- "操作前后联动 / 节点完成后更新表 / 流程钩子" → `--methods-json` + `--events-json` | 未明确要求时**不进入** workflow 阶段 |
...
| `scripts/lint_skill.py` | skill 仓库自检看门人（防止 skill 自身退化到老结构 / 已废弃字段）🆕 v1.4 |
...
3. **路径用绝对路径 + 严禁 metadata/ 层（spec v2 红线）**：
   - 调脚本前先 `path_resolver.py --apptag <tag>` 算好应用根目录绝对路径再传，不要让脚本去猜相对路径。
   - 应用根 = `<...>/META-INF/resources/<...>/<apptag>/`，**任何形如 `<apptag>/metadata/...` 的路径都是老结构**，spec v2 已废弃。
   - 计划文档里写"目标目录"时，禁止把 `metadata` 当作目录段拼进路径；写错的路径无法批准、调脚本会被 fail-fast 拦下。
   - 老应用迁移命令：`mv <apptag>/metadata/* <apptag>/ && rmdir <apptag>/metadata`。
